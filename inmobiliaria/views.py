...
from .models import Vendedor, Inquilino, Propietario, Propiedad, Reserva, Disponibilidad, ImagenPropiedad,Precio, TipoPrecio, Pago, ConceptoPago, HistorialDisponibilidad, VentaPropiedad, AlquilerMeses, Caja, MovimientoCaja, TipoMovimientoCaja, Movimiento, Sucursal
...
def reservar_alquiler_meses(request, propiedad_id):
    ...
    if request.method == 'POST':
        try:
            ...
            if not hasattr(propiedad, 'info_meses'):
                messages.error(request, 'La propiedad no tiene información de alquiler por meses.')
                return redirect('inmobiliaria:alquileres_24_meses')

            if propiedad.info_meses.estado != 'disponible':
            ...
            propiedad.info_meses.observaciones = request.POST.get('observaciones', '')
