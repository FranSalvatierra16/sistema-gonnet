...
from .models import Vendedor, Inquilino, Propietario, Propiedad, Reserva, Disponibilidad, ImagenPropiedad, Precio, TipoPrecio, Pago, ConceptoPago, HistorialDisponibilidad, VentaPropiedad, AlquilerMeses, Caja, MovimientoCaja, Cuenta, Concepto  # Added CuentaBancaria import and Movimiento import
...
def propiedad_nuevo(request):
    if request.method == 'POST':
        ...
        form = PropiedadForm(user=request.user)  # <-- ¡Agrega user=request.user!
        propietario_form = PropietarioForm(user=request.user)
