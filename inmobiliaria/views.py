            ...
            with transaction.atomic():
                ...
                if disponibilidad.fecha_inicio == fecha_inicio and disponibilidad.fecha_fin == fecha_fin:
                    ...
                    if disponibilidad.fecha_fin > fecha_fin:
                        # Crear disponibilidad para el período posterior
                        Disponibilidad.objects.create(
                            propiedad=propiedad,
                    ...
                    # Crear la disponibilidad para el período reservado
                    Disponibilidad.objects.create(
                        propiedad=propiedad,
                        fecha_inicio=fecha_inicio,
                        fecha_fin=fecha_fin,
...
def buscar_propiedades(request):
    ...
    if request.method == 'POST':
        form = BuscarPropiedadesForm(request.POST)
        if form.is_valid():
            ...
            # Aplicar filtros
            if form.cleaned_data.get('ambientes'):
                propiedades = propiedades.filter(ambientes=form.cleaned_data['ambientes'])
            ...
            for propiedad in propiedades:
                try:
                    ...
                    # Si hay una reserva pagada, saltar esta propiedad
                    if reservas.filter(estado='pagada').exists():
                        continue

                    # Verificar reserva en espera
                            ...
                            if disponibilidad.estado == 'disponible':
                                if (disponibilidad.fecha_inicio <= fecha_inicio and 
                                    ...
                                    for single_date in (fecha_inicio + timedelta(n) for n in range(dias_reserva)):
                                        tipo_precio = determinar_tipo_precio(single_date)
                                        try:
                                    ...
                                    # Asignar valores a la propiedad
                    ...
                    print(f"Error procesando propiedad {propiedad.id}: {str(e)}")
                    continue

    else:
        form = BuscarPropiedadesForm()
...
def determinar_tipo_precio(fecha):
...
def reserva_exitosa(request, reserva_id):
    ...
    return render(request, 'inmobiliaria/reserva/reserva_exitosa.html', {
        'reserva': reserva,
        'fecha_inicio': formato_fecha(reserva.fecha_inicio),
        'fecha_fin': formato_fecha(reserva.fecha_fin),
